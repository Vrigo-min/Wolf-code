...
/*
用法：
		1. DataScope_Get_Channel_Data(500*sin(a), 1 ); 将各变量数据，写入发送缓冲区
			DataScope_Get_Channel_Data(500* tan(a), 2 );
			DataScope_Get_Channel_Data( 500*cos(a), 3 );

		2. Send_Count = DataScope_Data_Generate(10);  完成后，用函数读取需要发送的数据量

		3. 循环Send_Count次将DataScope_OutPut_Buffer[i]中的内容发送出去。
			即完成了一次当前帧的数据上传。
